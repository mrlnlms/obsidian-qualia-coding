...
### Changed — popover de coding unificado cross-engine (placement + anchor tracker)

**`refactor(popover)` — markdown sai do CM6 native tooltip e passa a usar o mesmo `createPopover` floating dos outros engines.** Comportamento unificado em markdown, pdf, csv, image, audio, video:

- **Placement 4-way (`placeFloating` em `baseCodingMenu.ts`):** below → above → right → left. Escolhe o primeiro lado que cabe; se nenhum cabe, escolhe o de melhor relação espaço/popover. Clamp lateral ao viewport. Substitui o antigo `positionAndClamp` que só fazia flip vertical.
- **`AnchorSpec` com tracker opcional (`codingPopover.ts`):** scroll listener re-posiciona popover quando o conteúdo subjacente se move; fecha popover se anchor sair de view. Markdown translada por delta de scroll; image rastreia bbox da shape; csv rastreia bbox da célula via DOM connectedness; audio/video rastreia bbox da region.
- **`preferredSide` direcional:** markdown compara `mouseY` com centro do char range (seleção bottom-up → preferAbove); pdf compara `mouseY` com bbox de `window.getSelection()`. Cursor define a direção do popover, não estado interno do CM6.
- **Anchor = ponto do mouse no markdown** (não bbox da seleção). Em todos os lados o referencial é o cursor — "lado oposto do mouse, não da marcação". Em scroll, popover translada pelo `deltaScroll` do editor.
- **Audio/Video Y do mouse + X do `region.element`:** anchor híbrido. Capturei `mousedown` listener no `waveformEl` pra suprir falta de `MouseEvent` no callback `region-created` do wavesurfer.
- **ESC preserva seleção:** `view.focus()` no close do popover devolve foco pro editor (state.selection já sobrevivia, só faltava devolver foco).
- **Fix shell residual:** `rawClose` agora chama `options.onClose?.()` em todos os caminhos (ESC, click-outside, delete action) — antes o branch `externalContainer` deixava container vazio com `.menu` styling visível (sombra + border). Branch externalContainer removido como morto após o refactor.

Arquivos tocados: `baseCodingMenu.ts` (placeFloating + AnchorRect type), `codingPopover.ts` (AnchorSpec + tracker setup), `markdown/menu/menuController.ts` (handle floating popover), `markdown/menu/cm6NativeTooltipMenu.ts` (vira `buildMarkdownPopoverConfig`), `markdown/cm6/selectionMenuField.ts` (StateField/showTooltip removido, vira updateListener pra auto-close em seleção colapsada), `markdown/cm6/hoverMenuExtension.ts` (chama menuController direto), `image/views/imageView.ts` + `image/imageCodingMenu.ts` (AnchorSpec com tracker), `csv/csvCodingMenu.ts` (AnchorSpec via `anchorEl.getBoundingClientRect()` + tracker via `isConnected`), `csv/segmentEditor.ts` (menuController local), `media/mediaCodingMenu.ts` (anchor híbrido + lastMouseY), `media/mediaViewCore.ts` (mousedown capture no waveformEl), `pdf/pdfCodingMenu.ts` (preferredSide via getSelection). PDF e media sem tracker de scroll por hora — placement engine funciona, mas popover fica fixo na tela quando scrolla.

### Changed — poda de `!important` em styles.css (68 → 46)

**`refactor(css)` — categorização e remoção de `!important` redundantes via specificity.** Análise de 68 ocorrências em 5 categorias por inimigo real: A (7 cursor body overrides) + B (39 handles SVG transparency) ambos uso canônico, reclassificados como Permanente. C (18 AG Grid cells: headers + comment-cell + cod-seg-cell) **removidos** prefixando com `.ag-theme-quartz` pra subir specificity acima dos defaults AG Grid. D (2 SVG stroke em `.codemarker-pdf-handle circle` + `.codemarker-circle`) **removidos** redundantes — inimigo era atributo `stroke="white"` inline em SVG (presentation attribute, menor specificity, CSS sempre vence). E (2 isolados em kpi-board-btn hover + group memo convert-btn) **removidos** via aumento de especificidade pela hierarquia (`.codemarker-kpi-card:hover .codemarker-kpi-board-btn:hover` e `.codebook-groups-memo-wrap .codebook-groups-memo-convert-btn`). Smoke runtime validado em KPI hover (opacity vai pra 1 + cor accent) + group memo convert-btn alinhado + CSV cell rendering (headers + comment cells + cod-seg cells). Pattern documentado em `TECHNICAL-PATTERNS.md §49`. BACKLOG + ROADMAP recategorizados; gaps intra-modality ICR #1-4 marcados ✅ no Status Atual do ROADMAP (estavam abertos por inércia, já estavam fechados no §"Gaps descobertos em revisão de docs methodology" do BACKLOG).
...
**ICR Refactor C — Chunk C3 (UI: chip Distance + badge densidade + perCode rendering) (2026-05-13)** — branch `icr-refactor-c`. Terceira parte do set-valued labels refactor entrega UI. **Chip Distance** `[Jaccard][MASI]` no toolbar do Compare Coders ao lado dos chips de coeficiente; estado cinza condicional via predicate `isDistanceDisabled` (Cohen κ caminho A + α-binary são no-op; sem multi-label markers no escopo é no-op). **Badge densidade** `N/Total markers multi-label (X%)` sempre presente — comunica magnitude do efeito potencial. Tooltips educativos no chip + badge. **SavedComparison.view.distance** persistido em `data.json` com default `'jaccard'` (decisão D1 spec); leitura legada sem campo defaultar pra `'jaccard'`. **3 overview modes** (matrix/table/heatmap) propagam `state.distance` via cacheKey suffix `::δ-${name}` (§46: δ é parâmetro de comportamento, NÃO entra no scope key — entra no cacheKey separadamente). **perCode rendering em 2 lugares** (decisão cravada na sessão): (a) **tooltip da célula da matriz Mode A** ao hover mostra breakdown Cohen κ por código ordenado ascending; (b) **card no topo do drill-down Cards** com título "Decomposição por código" + lista codes ordenada por κ ascending, cor por faixa (qc-kappa-low/mid/good/high). Card só aparece quando `state.primaryCoefficient === 'cohen'` E há par selecionado em `currentSelection`. Card e tooltip alimentados pelo mesmo `CohenKappaReport.perCode` propagado em C2. **CSS novo:** `.qc-cc-distance-chip`, `.qc-cc-multilabel-badge`, `.qc-cc-percode-{card,header,list,item,value}`. **+24 tests** (3507 → 3530 → 3537 verde): 9 chip Distance + 2 badge + 7 smoke C3 cenários §4.4. Sem regressão. `multiLabelDensityInScope(scope, models)` helper exposto em `coderInclusion.ts` itera markers de todas engines respeitando filters de scope.
