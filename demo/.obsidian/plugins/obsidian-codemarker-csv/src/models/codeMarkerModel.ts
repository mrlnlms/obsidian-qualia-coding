...
export class CodeMarkerModel {
	...
	async loadMarkers() {
		...
		// If no markdown markers in own data.json, try importing from codemarker-v2
		if (!data?.markers) {
			try {
				const v2Path = this.plugin.app.vault.configDir + '/plugins/obsidian-codemarker-v2/data.json';
				const adapter = this.plugin.app.vault.adapter;
				if (await adapter.exists(v2Path)) {
					const v2Raw = await adapter.read(v2Path);
					const v2Data = JSON.parse(v2Raw);
					if (v2Data?.markers) {
						console.log('[CodeMarker CSV] Importing markdown markers from codemarker-v2');
						if (!data) data = {};
						data.markers = v2Data.markers;
						data.codeDescriptions = v2Data.codeDescriptions ?? data.codeDescriptions;
						data.codeDefinitions = v2Data.codeDefinitions ?? data.codeDefinitions;
						data.nextPaletteIndex = v2Data.nextPaletteIndex ?? data.nextPaletteIndex;
					}
				}
			} catch (e) {
				console.warn('[CodeMarker CSV] Could not import from codemarker-v2:', e);
			}
		}
