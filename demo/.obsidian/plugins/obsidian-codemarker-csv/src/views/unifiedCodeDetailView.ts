...
export class UnifiedCodeDetailView extends ItemView {
	...
	private csvModel: CodingModel;
	private markerId: string | null = null;
	private codeName: string | null = null;
	private changeListener: () => void;

	constructor(leaf: WorkspaceLeaf, mdModel: CodeMarkerModel, csvModel: CodingModel) {
		super(leaf);
	...
	async onOpen() {
		...
		this.csvModel.onChange(this.changeListener);
		this.refreshCurrentMode();
	}

	async onClose() {
		this.csvModel.offChange(this.changeListener);
	...
	// ─── Marker Origin Detection ────────────────────────────

	/** Try to find a marker by id in either model. Returns { source, marker }. */
	private findMarker(id: string): { source: 'md'; marker: Marker } | { source: 'csv'; marker: CsvMarker } | null {
		const mdMarker = this.mdModel.getMarkerById(id);
		if (mdMarker) return { source: 'md', marker: mdMarker };

		const csvMarker = this.csvModel.findMarkerById(id);
		if (csvMarker) return { source: 'csv', marker: csvMarker };
	...
	private renderList() {
		...
		// Use shared registry (both models share registry via shared file)
		const codes = this.mdModel.registry.getAll();
		const counts = this.countSegmentsPerCodeCombined();
	...
	private countSegmentsPerCodeCombined(): Map<string, number> {
	...
	private renderCodeDetail() {
		...
		// CSV markers
		const csvMarkers = this.csvModel.getAllMarkers()
			.filter(m => m.codes.includes(this.codeName!));

		if (mdMarkers.length === 0 && csvMarkers.length === 0) {
		...
		if (mdMarkers.length > 0) {
			...
			for (const marker of mdMarkers) {
				...
				li.createSpan({ cls: 'codemarker-detail-marker-file', text: this.shortenPath(marker.fileId, '.md') });
		...
		if (csvMarkers.length > 0) {
			...
			for (const marker of csvMarkers) {
				...
				const li = listEl.createEl('li', { cls: 'codemarker-detail-marker-item' });
				li.createSpan({ cls: 'codemarker-detail-marker-file', text: this.shortenPath(marker.file, '.csv') });
				li.createEl('span', { text: preview });
	...
	private renderMarkerDetail() {
		...
		const found = this.findMarker(this.markerId);
		if (!found) {
		...
		if (found.source === 'md') {
			this.renderMdMarkerDetail(container, found.marker as Marker);
		} else {
			this.renderCsvMarkerDetail(container, found.marker as CsvMarker);
		}
	}

	private renderMdMarkerDetail(container: HTMLElement, marker: Marker) {
		// Text segment
		const text = this.getMdMarkerText(marker);
		if (text) {
		...
		// Other markdown markers with same code
	...
	private renderCsvMarkerDetail(container: HTMLElement, marker: CsvMarker) {
		// Text segment
		const text = this.csvModel.getMarkerText(marker);
		if (text) {
			const textSection = container.createDiv({ cls: 'codemarker-detail-section' });
		...
		locSection.createEl('p', {
			text: this.csvModel.getMarkerLabel(marker) + ' · ' + this.shortenPath(marker.file, '.csv'),
		...
		// Other CSV markers with same code
		const otherMarkers = this.csvModel.getAllMarkers()
			.filter(m => m.id !== marker.id && m.codes.includes(this.codeName!));
		if (otherMarkers.length > 0) {
			const markersSection = container.createDiv({ cls: 'codemarker-detail-section' });
			markersSection.createEl('h6', { text: 'Other Markers' });
			...
			for (const other of otherMarkers) {
				...
				const li = markerList.createEl('li', { cls: 'codemarker-detail-marker-item' });
				li.createSpan({ cls: 'codemarker-detail-marker-file', text: this.shortenPath(other.file, '.csv') });
				li.createEl('span', {
	...
	private async navigateToCsvMarker(marker: CsvMarker) {
