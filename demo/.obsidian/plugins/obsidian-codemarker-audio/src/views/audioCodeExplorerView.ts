/**
 * Audio Code Explorer — tree view with 3 levels: Code → File → Segment.
...
export const AUDIO_CODE_EXPLORER_VIEW_TYPE = 'codemarker-audio-explorer';
...
export class AudioCodeExplorerView extends ItemView {
	...
	getViewType(): string {
		return AUDIO_CODE_EXPLORER_VIEW_TYPE;
	}

	getDisplayText(): string {
		return 'Audio Code Explorer';
	}
	...
	render() {
		...
		if (codeIndex.size === 0) {
			container.createEl('p', {
				text: 'No codes yet. Open an audio file and drag to select a region.',
				cls: 'pane-empty',
			});
			return;
		}
	...
	private buildCodeIndex(): Map<string, Map<string, AudioMarker[]>> {
		const index = new Map<string, Map<string, AudioMarker[]>>();
