/**
 * Video Code Detail View — sidebar with 3 modes: list, code-focused, marker-focused.
...
export const VIDEO_CODE_DETAIL_VIEW_TYPE = 'codemarker-video-detail';

const VIDEO_EXTS = /\.(mp3|m4a|wav|ogg|flac|aac)$/i;

export class VideoCodeDetailView extends ItemView {
	...
	getViewType(): string {
		return VIDEO_CODE_DETAIL_VIEW_TYPE;
	}

	getDisplayText(): string {
		if (this.codeName) return this.codeName;
		return 'Video Code Detail';
	}
	...
	private renderCodeDetail() {
		...
		const allMarkers: { marker: VideoMarker; filePath: string }[] = [];
	...
	private renderMarkerDetail() {
		...
		const otherMarkers: { marker: VideoMarker; filePath: string }[] = [];
		for (const af of this.model.files) {
			for (const m of af.markers) {
