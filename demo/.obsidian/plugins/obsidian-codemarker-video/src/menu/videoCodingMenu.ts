...
import type { VideoRegionRenderer } from '../video/regionRenderer';
import { CodeFormModal } from './videoCodeFormModal';
...
export function openVideoCodingPopover(
	...
	regionStart: number,
	regionEnd: number,
	regionRenderer: VideoRegionRenderer,
	onDismissEmpty: () => void,
	app: App,
	...
	document.querySelector('.codemarker-video-popover')?.remove();

	const container = document.createElement('div');
	container.className = 'menu codemarker-video-popover';
	...
	const rebuild = () => {
		close();
		openVideoCodingPopover(mouseEvent, model, filePath, regionStart, regionEnd, regionRenderer, onDismissEmpty, app, onNavigate, pos);
	};
	...
	for (const codeDef of allCodes) {
		...
		swatch.className = 'codemarker-video-popover-swatch';
		...
		if (isActive && onNavigate && existingMarker) {
			...
			navBtn.className = 'codemarker-video-popover-nav';
	...
	if (existingMarker && existingMarker.codes.length > 0) {
		...
		memoWrapper.className = 'codemarker-video-memo-wrapper';
		const memoArea = document.createElement('textarea');
		memoArea.className = 'codemarker-video-memo';
