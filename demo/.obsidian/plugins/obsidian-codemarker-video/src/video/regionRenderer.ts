...
export class VideoRegionRenderer {
	...
	constructor(renderer: WaveformRenderer, model: VideoCodingModel) {
	...
	renderMarkerRegion(marker: VideoMarker): void {
		...
		if (this.model.settings.showLabelsOnRegions && marker.codes.length > 0) {
			...
			content.className = 'codemarker-video-region-label';
	...
	private applyHoverToRegions(markerId: string | null): void {
		for (const [mid, region] of this.markerToRegion) {
			...
			if (!el) continue;
			el.classList.toggle('codemarker-video-region-hovered', markerId === mid);
		}
	}
	 ...
	 */
	private renderMinimapMarkers(markers: VideoMarker[]): void {
		...
		for (const marker of markers) {
			...
			bar.className = 'codemarker-video-minimap-marker';
			bar.style.left = `${left}%`;
	 ...
	 */
	private applyLanes(markers: VideoMarker[]): void {
