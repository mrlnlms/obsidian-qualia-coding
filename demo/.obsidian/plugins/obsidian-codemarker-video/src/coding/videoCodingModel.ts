...
import type { VideoMarker, VideoFile, VideoPluginData, VideoSettings } from './videoCodingTypes';
...
export class VideoCodingModel {
	...
	async load(): Promise<void> {
		...
		if (raw) {
			...
			this.settings = { ...DEFAULT_VIDEO_SETTINGS, ...raw.settings };
		}
	...
	findOrCreateMarker(filePath: string, from: number, to: number): VideoMarker {
		...
		const af = this.getOrCreateVideoFile(filePath);
