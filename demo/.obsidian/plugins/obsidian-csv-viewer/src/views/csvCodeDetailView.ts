...
import { CodingModel, CsvMarker, isSegmentMarker } from '../coding/codingModel';
...
export class CsvCodeDetailView extends ItemView {
	...
	async onOpen() {
		...
		this.registerEvent(
			this.app.workspace.on('codemarker-csv:model-changed' as any, () => {
				if (this.markerId && this.codeName) this.render();
				else if (this.codeName) this.renderCodeDetail();
	...
	private renderCodeDetail() {
		...
		for (const marker of allMarkers) {
			...
			fileRef.textContent = this.shortenPath(marker.file) + ' \u2014 ' + label;
	...
	private render() {
		...
		locSection.createEl('p', {
			text: `${this.shortenPath(marker.file)} \u2014 ${this.model.getMarkerLabel(marker)}`,
		...
		const otherMarkers = this.model.getMarkersForFile(marker.file)
