/**
 * Audio coding popover — thin wrapper around the shared openCodingPopover().
 *
 * Audio-specific logic: region-based marker resolution (by start/end time),
 * region refresh after toggle, dismiss-empty cleanup.
 */
...
export function openAudioCodingPopover(
	...
	// Lazy marker — created on first code add
	...
	const options: CodingPopoverOptions = {
		...
		onClose: () => {
			// If marker has no codes after close, clean up
		...
		onRebuild: () => {
			openAudioCodingPopover(mouseEvent, model, filePath, regionStart, regionEnd, regionRenderer, onDismissEmpty, app, pos);
		},
