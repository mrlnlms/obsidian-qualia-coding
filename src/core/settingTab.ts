...
export class QualiaSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.createEl('p', {
			text: 'Folders where memos materialize as .md notes. All four types active: Code, Group, Marker, and Relation (code-level + application-level).',
		...
		new Setting(containerEl)
			...
			.setDesc('Path inside vault where marker (segment) memos save when materialized.')
			.addText(t => t
				.setValue(generalSettings.memoFolders.marker)
				.onChange((v) => {
					generalSettings.memoFolders.marker = v.trim();
					save();
				}));

		new Setting(containerEl)
			.setName('Relation memo folder')
			.setDesc('Path inside vault where relation memos save when materialized (code-level and application-level).')
			.addText(t => t
				.setValue(generalSettings.memoFolders.relation)
				.onChange((v) => {
					generalSettings.memoFolders.relation = v.trim();
					save();
				}));
		...
		new Setting(containerEl)
			...
			.addButton(btn => btn
				...
				.onClick(() => {
					new ExportModal(this.plugin, this.plugin.dataManager, this.plugin.sharedRegistry, 'qdpx', this.plugin.manifest.version, this.plugin.caseVariablesRegistry).open();
				}));
		...
		new Setting(containerEl)
			...
			.addButton(btn => btn
				...
				.onClick(() => {
					new ExportModal(this.plugin, this.plugin.dataManager, this.plugin.sharedRegistry, 'qdc', this.plugin.manifest.version, this.plugin.caseVariablesRegistry).open();
				}));
