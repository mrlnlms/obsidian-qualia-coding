...
| Pattern | Evidence | Impact |
|---------|----------|--------|
| MAXQDA-style margin bars | `marginPanelExtension.ts` (539 LOC) — column allocation, label collision avoidance | Coding density visible in peripheral vision |
| Column ∝ overlap count | `assignColumns()`: more overlapping markers → more columns → wider panel | Panel width encodes complexity |
| Larger bars → inner columns | Sort by span descending; largest bars closest to text (`marginPanelExtension.ts:481-511`) | Most important context stays near content |
...
| Type guards for polymorphism | `isPdfMarker()`, `isImageMarker()`, `isCsvMarker()`, `isAudioMarker()`, `isVideoMarker()` in unified views | Engine-specific rendering without `switch` cascades |
...
**Column allocation** (`marginPanelExtension.ts:481-511`):
...
**Label collision avoidance** (`marginPanelExtension.ts:518-559`):
...
Defined in `unifiedExplorerView.ts:150-168` and `unifiedDetailView.ts:175-193`:
...
Used in `getMarkerLabel()` and `navigateToMarker()` for engine-specific rendering and navigation.
...
| `src/core/unifiedExplorerView.ts` | Type guards, navigation events, marker labels |
...
| `src/markdown/cm6/marginPanelExtension.ts` | Collision avoidance, column allocation, RLL |
