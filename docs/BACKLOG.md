...
- [x] **Lentidão Compare Coders pós-E5a — INVESTIGADA E FIXADA 2026-05-11**: `getCodersWithMarkersInScope` iterava 7 engines (com spread alocador) em todo `renderToolbar`; `collectContestedRegions` iterava 5 engines em todo `renderDrilldown` sem cache. Ambos cacheados (LRU 50 + gen counter, pattern do extractInputsFromScope). Iteração inline por engine substitui spread. Bump unificado via `bumpAllIcrCaches()` em mutações relevantes.
