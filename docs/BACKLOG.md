...
- [x] **Lentidão Compare Coders pós-E5a — INVESTIGADA E FIXADA 2026-05-11**: 4 caches em camadas medidos via instrumentation (toolbar=1-5ms, overview=1200ms→1-3ms em cache hit). (1) `getCodersWithMarkersInScope` cacheado (era spread de 7 engines em todo `renderToolbar`); (2) `collectContestedRegions` cacheado (5 engines + clustering em todo drilldown); (3) `reportKappa`/`reportPairwise` cacheados em 2 camadas (WeakMap identity + Map scope-key) — antes troca de coefficient recomputava 150× explodeMarkersToCharLabels; (4) `extractInputsFromScope` ganhou cache per-engine (chip de filter não invalida cache de outros engines); (5) Heatmap + Table paralelizaram `await` sequenciais via `Promise.all`; (6) `explodeMarkersToCharLabels` memoizado por identity (5 coefs dentro de computeAll só pagam 1×). Limites: combo nunca-vista paga primeira passada real (400-1900ms — trabalho computacional verdadeiro dos coefs); pre-warm em background ou worker thread ficaria pra refactor maior se vier reclamação real. Bump unificado via `bumpAllIcrCaches()`.
...
- [ ] **IcrMarkerOps: PDF text + CSV segment + audio + video + image + pdfShape** — bounds.kind='text' / 'temporal' são insuficientes pra essas engines (PDF precisa page+spans, CSV-segment precisa sourceRowId+column+from/to, audio/vídeo precisa file durations). Slice E3a Fase 1 cobre só markdown + csvRow. Estender bounds com variants engine-specific (`{kind:'pdfText', page, beginIndex, ...}` / `{kind:'csvSegment', rowIndex, column, from, to}`) — temporal já existe e cobre audio/video; bbox novo pra image/pdfShape.
