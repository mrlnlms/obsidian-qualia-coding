...
**Dívida técnica aberta:** Sets de extensão duplicados entre `*View.ts` e `*/index.ts` (AUDIO_EXTENSIONS / VIDEO_EXTENSIONS / IMAGE_EXTENSIONS). Cada um está no lugar natural (view declara via `canAcceptExtension`; index filtra menu/rename). Consolidar se virar dor — não vale tocar preventivamente.
