...
- [ ] **Investigar lentidão observada Compare Coders View pós-E5a** — smoke real 2026-05-11 reportou lentidão perceptível em interações de heatmap → drill-down. Possível causa: `collectContestedRegions` agora itera 5 engines em vez de 2, sem cache. Verificar se vale memoizar com chave (state.scope.coderIds + scopeKey + generation counter), ou se é apenas perf de hardware do dev box. Não bloqueante — features funcionam.
