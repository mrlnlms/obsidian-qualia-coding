...
| ~~E4~~ | ~~FEITO~~ | `core/imageDimensions.ts` | ~~Util compartilhada `getImageDimensions(vault, filePath)` com fallback `createImageBitmap` → `<img>` decode. SVG validado em teste manual (round-trip export/import com dimensões corretas). BMP presumivelmente ok. TIFF/HEIC ainda não suportados — Electron os delega pro OS, Obsidian nem consegue renderizar, fora do alcance do plugin. Usada por `qdpxExporter` e `qdpxImporter`.~~ (2026-04-22) |
