...
**Cross-cutting remanescente:** `!important` recategorizado 2026-05-13 (ver §Cross-cutting pendente abaixo). 7 cursor overrides movidos pra Permanente (uso canônico); 2 isolados removíveis baratos; 46 handles SVG + 15 AG Grid cell overrides ficam (defesa legítima contra background herdado e `.ag-cell *` specificity, não dívida).
...
|------|----------------------------------------|
| **`styles.css` `!important` — recategorizado e podado 2026-05-13.** 68 → 46 (22 removidos). Análise dividida em 5 categorias por inimigo real: **A (7 cursor body overrides)** legítimos — movidos pra §Permanente, uso canônico não-removível. **B (39 handles SVG transparency, linhas 833-987 + 1151)** — defesa contra `.codemarker-highlight` ter `background-color` inline da cor do código; handles SVG dentro herdariam. Re-arquitetura possível mas sem ganho funcional, fica como §Permanente. **C (15 AG Grid cells, linhas 1233-1287)** — ✅ **REMOVIDOS 2026-05-13** prefixando com `.ag-theme-quartz .ag-cell` / `.ag-theme-quartz .ag-header-cell` pra subir specificity acima dos defaults AG Grid. Inclui consolidação de `.ag-cell.csv-comment-cell-nowrap` em uma única regra. **D (2 SVG stroke, linhas 776 + 987)** — ✅ **REMOVIDOS 2026-05-13** redundantes: inimigo era atributo `stroke="white"` inline em SVG (presentation attribute), CSS sempre vence sem `!important`. **E (2 isolados, linhas 3477 + 5655)** — ✅ **REMOVIDOS 2026-05-13** via especificidade (kpi-board-btn hover + group memo convert-btn). **Smoke validado:** KPI hover + group memo convert botão alinhados (E) + CSV cell rendering (C). |
...
| ~~**Canvas (outros engines) não refresha cor quando cor do code muda**~~ ✅ **RESOLVIDO release 0.7.0 (2026-05-13)** — todos os 5 engines (image + markdown + pdf-text/shape + csv + audio/video) fecham o caso. Pattern unificado: subscribe `qualia:registry-changed` (document event) + `model.onChange` em paralelo, ambos disparando refresh com rAF coalesce. PDF-shape, markdown e CSV ganharam também precedência de `marker.colorOverride` sobre cor do code nos renderers afetados (`markerStateField`, `marginPanelExtension`, `drawLayer`, `marginPanelRenderer`). CSV cellRenderer é per-code (chip), não per-marker — colorOverride não tem mapping visual claro; decisão consciente de não expor. Ver CHANGELOG 0.7.0 §"Canvas refresh cor cross-engine". |
