...
|------|------------------|
| **[Coding Management](#2-coding-management)** | Tier 1 ✅ FEITO 2026-04-28. Tier 2: code merging avançado (audit log entregue) · Tier 3 (bloqueado por LLM): Smart Codes |
| **[Analytics](#3-analytics--melhorias)** | Multi-tab xlsx export · Codebook timeline central (consome audit log) |
...
| **Code merging avançado** | Merge interativo: preview rico (markers/groups/filhos afetados), escolher nome+cor mantido (radio com swatches), política explícita pra memos/descriptions (discard/concatenate/keep specific) | Hoje merge é simples (descarta memos dos sources sem aviso). MergeModal 70% pronto — falta UI dos novos inputs + lógica em `executeMerge` |
...
| **Codebook timeline central** | 4-5h MVP / 7-9h full | Mode novo no Analytics que consome `data.auditLog` (#29) e mostra cronologia agregada de TODAS as decisões do codebook (renames, merges, edits) cross-código. **MVP curto:** lista cronológica filtrável (por tipo de evento, por code) + click navega pro Code Detail + export markdown agregado. **Full:** stacked bar chart por dia colorido por tipo + agregação por granularidade (dia/semana/mês). Distinto do Temporal mode existente — Temporal mostra aplicações de markers (`marker.createdAt`); Codebook timeline mostra **decisões analíticas** (auditLog). |
