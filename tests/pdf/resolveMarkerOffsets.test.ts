...
describe('resolveMarkerOffsets', () => {
	it('resolve offset absoluto em página única (page 1, 1-based)', () => {
		const result = resolveMarkerOffsets('hello world foo', [0], { page: 1, text: 'world' });
		expect(result).toEqual({ start: 6, end: 11, ambiguous: false });
	});

	it('resolve offset absoluto na page 2 (1-based) de PlainText multi-página', () => {
		const result = resolveMarkerOffsets('page one\fpage two', [0, 9], { page: 2, text: 'two' });
	...
	it('sinaliza ambiguidade quando text aparece múltiplas vezes na página', () => {
		const result = resolveMarkerOffsets('foo bar foo', [0], { page: 1, text: 'foo' });
		expect(result).toEqual({ start: 0, end: 3, ambiguous: true });
	});

	it('retorna null quando text não existe na página', () => {
		const result = resolveMarkerOffsets('hello', [0], { page: 1, text: 'xyz' });
		expect(result).toBeNull();
	});

	it('retorna null quando page está fora do range', () => {
		const result = resolveMarkerOffsets('hello', [0], { page: 99, text: 'hello' });
		expect(result).toBeNull();
	});
	...
	it('whitespace normalization também funciona com newlines/tabs', () => {
		...
		const result = resolveMarkerOffsets(plainText, [0], { page: 1, text: 'hello world' });
		expect(result).not.toBeNull();
	});
});
